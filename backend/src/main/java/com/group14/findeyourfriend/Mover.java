...
public class Mover
{
	...
	{
		if ((getPosition().getCoordinates().x + getAcceleration().x) > Constants.MAX_WIDTH || (getPosition().getCoordinates().x + getAcceleration().x) < Constants.MIN_WIDTH)
		{
			Turn180DegreesOnXAxis();
		}
		if ((getPosition().getCoordinates().y + getAcceleration().y) > Constants.MAX_HEIGHT || (getPosition().getCoordinates().y + getAcceleration().y) < Constants.MIN_HEIGHT)
		{
